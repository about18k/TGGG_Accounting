...
export default function SiteCoordinatorDashboard({ user, onNavigate }) {
  ...
  const attendanceRows = [
    { date: '2026-02-09', in: '08:00', out: '17:00', late: '0', hours: '8h 0m', note: 'Coordinated deliveries and updated site logs.' },
    { date: '2026-02-08', in: '08:11', out: '17:00', late: '11', hours: '7h 49m', note: 'Managed schedule alignment and team handoff.' },
  ];
  ...
  const renderOverview = () => (
    <div className={cardClass}>
      <div className="p-6 border-b border-white/10">
        <h1 className="text-2xl font-semibold text-white">Site Coordinator Dashboard</h1>
        <p className="text-white/60 text-sm mt-1">Track attendance, coordination tasks, and material requests.</p>
      </div>
      <div className="p-6 space-y-6">
        <div className={`${cardClass} p-6`}>
          <h3 className="text-white text-lg font-semibold">Site Coordinator Responsibilities</h3>
          <ul className="mt-3 space-y-2 text-sm text-white/75">
            <li>• Coordinates site activities across teams and suppliers.</li>
            <li>• Tracks material movement and request timelines.</li>
            <li>• Maintains site communication and daily records.</li>
            <li>• Ensures smooth handoffs and schedule alignment.</li>
          </ul>
        </div>
        <div className={`${cardClass} p-6`}>
          <div className="flex items-center gap-3">
            <Users className="h-5 w-5 text-[#FF7120]" />
    ...
    <div className="space-y-5 sm:space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className={`${cardClass} p-4 sm:p-6 space-y-3`}>
          ...
          <textarea rows={5} value={workDoc} onChange={(e) => setWorkDoc(e.target.value)} placeholder="What coordination work did you accomplish today?" className="w-full rounded-xl border border-white/15 bg-[#00273C]/60 px-3 py-2 text-sm text-white placeholder:text-white/45 outline-none resize-none" />
      ...
      <div className="relative pt-28 px-6 pb-10">
        <div className="max-w-[1600px] mx-auto flex gap-6">
          <aside className="w-64 shrink-0 hidden lg:block">
            <SiteCoordinatorSidebar currentPage="attendance" onNavigate={onNavigate} activeSection={activeSection} onSelectSection={setActiveSection} />
          </aside>
