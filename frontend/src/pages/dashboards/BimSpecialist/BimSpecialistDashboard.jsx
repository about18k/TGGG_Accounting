...
export default function BimSpecialistDashboard({ user, onNavigate }) {
  ...
  const formatHoursAndMinutes = (decimalHours) => {
    ...
    if (hours > 0 && minutes > 0) return `${hours}${hourLabel} and ${minutes} ${minuteLabel}`;
        ...
        {isAttendanceTotalsOpen && (
          <div className="px-4 sm:px-6 pb-4 sm:pb-6 space-y-4 border-t border-white/10">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
              ...
              <label className="flex flex-col gap-1 pt-2 text-xs text-white/70">
                Start Date
                <input type="date" value={rangeStartDate} onChange={(e) => setRangeStartDate(e.target.value)} className="rounded-lg border border-white/15 bg-[#001f35] px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF7120]/60 scheme-dark" />
              </label>
              <label className="flex flex-col gap-1 pt-2 text-xs text-white/70">
                End Date
                <input type="date" value={rangeEndDate} onChange={(e) => setRangeEndDate(e.target.value)} className="rounded-lg border border-white/15 bg-[#001f35] px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF7120]/60 scheme-dark" />
              </label>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
              <div className="rounded-xl border border-white/10 bg-background/70 p-3"><p className="text-xs text-white/60">Total Hours</p><p className="mt-1 text-2xl font-semibold text-white">{formatHoursAndMinutes(attendanceTotals.totalHours)}</p></div>
              ...
              <div className="rounded-xl border border-white/10 bg-background/70 p-3"><p className="text-xs text-white/60">Total Late</p><p className="mt-1 text-2xl font-semibold text-white">{formatHoursAndMinutes(attendanceTotals.totalLate)}</p></div>
              <div className="rounded-xl border border-white/10 bg-background/70 p-3"><p className="text-xs text-white/60">Total Overtime Worked</p><p className="mt-1 text-2xl font-semibold text-white">{formatHoursAndMinutes(attendanceTotals.totalOvertimeHours)}</p></div>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#00273C] relative overflow-x-hidden">
