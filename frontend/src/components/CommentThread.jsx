 ...
 */
const ReplyItem = ({ reply, docId, currentUserId, onRefresh }) => {
    ...
    const handleEdit = async () => {
        ...
        setSaving(true);
        try {
            const result = await bimDocumentationService.editComment(docId, reply.id, text);
            if (result.success) {
                setIsEditing(false);
                onRefresh();
            }
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            const result = await bimDocumentationService.deleteComment(docId, reply.id);
            if (result.success) {
                onRefresh();
            }
        } finally {
 ...
 */
const CommentItem = ({ comment, docId, currentUserId, onRefresh }) => {
    ...
    const submitReply = async () => {
        ...
        setPosting(true);
        try {
            const result = await bimDocumentationService.postComment(docId, text, comment.id);
            if (result.success) {
                setReplyText('');
    ...
    const handleEdit = async () => {
        ...
        setSaving(true);
        try {
            const result = await bimDocumentationService.editComment(docId, comment.id, text);
            if (result.success) {
                setIsEditing(false);
                onRefresh();
            }
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            const result = await bimDocumentationService.deleteComment(docId, comment.id);
            if (result.success) {
                onRefresh();
            }
        } finally {
            ...
            {comment.replies && comment.replies.length > 0 && (
                <div className="ml-11 space-y-2 border-l-2 border-white/10 pl-3">
                    {comment.replies.map((reply) => (
                        <ReplyItem key={reply.id} reply={reply} docId={docId} currentUserId={currentUserId} onRefresh={onRefresh} />
                    ))}
                </div>
            )}
...
/**
 * Full comment thread for one BIM documentation document.
 *
 * Props:
...
const CommentThread = ({ docId, currentUser, collapsible = false, defaultOpen = true }) => {
    ...
    const loadComments = useCallback(async () => {
        if (!docId) return;
        setLoading(true);
        const result = await bimDocumentationService.getComments(docId);
    ...
    const submitComment = async () => {
        ...
        setPosting(true);
        try {
            const result = await bimDocumentationService.postComment(docId, text);
            if (result.success) {
                setNewComment('');
                loadComments();
            }
